function [fixationstats,trialtype] = ClusterFixation(datafile,samprate)
% Copyright 2013 Seth Koenig (skoenig3@uw.edu), Buffalo Lab
% Setup to run for BCRW analysis/SCM data sets
...
[eyedat,trialtype]  = getSCM_EyeData(datafile,samprate);
fixationstats = cell(1,length(eyedat));
for cndlop = 1:length(eyedat)
    if length(eyedat{cndlop}) < 200
        %added if statement 5/20/15 SDK
        continue
    end
