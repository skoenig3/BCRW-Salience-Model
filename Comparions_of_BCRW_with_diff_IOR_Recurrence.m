...
IOR_taus = [0 30 25 20 17 14 12 9 7 5 3 1];
...
for imset = 1:length(image_sets);
    ...
    for IOR = 1:length(IOR_taus);
        for img = 1:36;
            ...
            for t = 1:length(tags)
                if IOR_taus(IOR) == 0
                    load([dirName IOR_dir tags{t} '-' num2str(img) '-' num2str(IOR_taus(IOR)) '-BCRW.mat'])
                else
                    load([dirName IOR_dir tags{t} '-' num2str(img) '-' num2str(1/IOR_taus(IOR)) '-BCRW.mat'])
                end
                 BCRWfixations = fixationtimes;
                
                %---Calculate Salience at BCRW Fixations---%
                corm = NaN(2,size(BCRWfixations,1),1);
            ...
            all_corms{IOR}(:,index) = nanmean(corm,2);
        end
    end
end
%%
...
figure
for it = 1:2:length(IOR_taus)
    subplot(2,3,(it+1)/2)
    h = imagesc(normalized_all_recurrence{it});
    axis xy, set(h,'alphadata',~isnan(normalized_all_recurrence{it}));
    box off
    axis square
    xlim([0.5 35])
    ylim([0.5 35])
    title(['IOR_tau = ' num2str(IOR_taus(it))])
    colormap('viridis')
end
...
xlabl = {};
for IOR = 1:length(IOR_taus)
    all_coms = [all_coms  all_corms{IOR}(1,:)-all_corms{IOR}(2,:)];
    all_its = [all_its IOR*ones(1,size( all_corms{IOR},2))];
    xlabl = [xlabl {['tau 1/' num2str(IOR_taus(IOR))]}];
...
ylabel('Center of Mass,Local->Global')
ylim([3 4])