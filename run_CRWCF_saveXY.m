function run_CRWCF_saveXY(CombinedViewingBehaviorData,saliencemapfile,tagname,imageX,imageY,plotoptions,IOR_tau)
% Function made by Seth Koenig on 5/17/2016. 
% Same as  run_CRWCF_saveXY but no salience bias so it's a CRW not a BCRW
...
%---No More Bias---%
%if loading salience map this is what would run. 
...
%instead setting saliencemap all to 1 and gradients to 0

saliencemap = zeros(imageY,imageX); %instead of changing persistence were going to add a random direction see
fx = zeros(imageY,imageX);
fy = zeros(imageY,imageX);
...
if IOR_tau == 0;
    BCRWfolder = 'CRW IOR TAU 0\';
else
    BCRWfolder = ['CRW IOR TAU ' num2str(1/IOR_tau) '\'];
end
if ~exist(BCRWfolder,'dir')
    mkdir(BCRWfolder)
end

save([BCRWfolder tagname '-' saliencemapfile(1:dash-1) '-CRW.mat'],'alltrials',...
